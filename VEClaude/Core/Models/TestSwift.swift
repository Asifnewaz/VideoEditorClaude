...
//  VEClaude
//
//  Created by BCL Device 24 on 23/8/25.
//
...
/*
 ...
 class VideoTimelineView: UIView {
     ...
     func handleLeftEarPan(_ gesture: UIPanGestureRecognizer, selectionView: VideoSelectionView) {
         ...
         case .changed:
             // Calculate new left ear position (like right ear behavior)
             let currentLeftX = selectionView.frame.origin.x
             let newLeftX = currentLeftX + translation.x
             
             // Calculate boundaries
             let originalContainerStartX = originalStartPosition
             let currentRightEarX = selectionView.frame.origin.x + selectionView.frame.width - 20 // Where right ear starts
             let maxLeftEarX = currentRightEarX - 60 - 20 // Minimum 1 second content + left ear width
             
             // Always limit expansion to original duration - no video can exceed its original length
             let minLeftEarX = originalContainerStartX - 20 // Minimum at original video start, never beyond
             
             print("LeftEar Debug - currentLeftX: \(currentLeftX), newLeftX: \(newLeftX)")
             print("LeftEar Debug - minLeftEarX: \(minLeftEarX), maxLeftEarX: \(maxLeftEarX)")
             
             // Constrain the left ear position
             let clampedLeftX: CGFloat
             if newLeftX < minLeftEarX {
                 clampedLeftX = minLeftEarX
                 print("LeftEar Debug - Constrained to minimum")
             } else if newLeftX > maxLeftEarX {
                 clampedLeftX = maxLeftEarX
                 print("LeftEar Debug - Constrained to maximum")
             } else {
                 clampedLeftX = newLeftX
                 print("LeftEar Debug - No constraint applied")
             }
             
             // Calculate new selection width and position
             let requestedSelectionWidth = (selectionView.frame.origin.x + selectionView.frame.width) - clampedLeftX
             ...
             print("LeftEar Debug - requestedSelectionWidth: \(requestedSelectionWidth), constrainedSelectionWidth: \(constrainedSelectionWidth)")
             
             // Update selection view with new position and constrained width
             let newSelectionFrame = CGRect(
                 x: clampedLeftX,
                 y: selectionView.frame.origin.y,
                 width: constrainedSelectionWidth,
                 height: selectionView.frame.height
             )
             selectionView.updateFrame(newSelectionFrame)
             
             // Calculate container clipping from left: change both position and width
             // Thumbnails inside stay in their original positions but get clipped from left
             let requestedContentWidth = constrainedSelectionWidth - 40 // Account for 20px ears on each side
             let effectiveContentWidth = originalWidth //min(requestedContentWidth, originalWidth) // Never exceed original width
             let clippedWidth = effectiveContentWidth
             
             // Calculate how much to move container position from left for clipping
             let leftTrimAmount = max(0, (clampedLeftX + 20) - originalStartPosition)
             
             // Calculate trim amount from left based on left ear position
             let totalLeftTrimAmount = max(0, (clampedLeftX + 20) - originalStartPosition)
             
             // Update container frame: keep X position fixed, only reduce width (like right ear)
             selectedContainer.frame = CGRect(
                 x: selectedContainer.frame.origin.x, // Keep container X position unchanged
                 y: selectedContainer.frame.origin.y,
                 width: effectiveContentWidth, // Reduce width based on selection
                 height: selectedContainer.frame.height
             )
             
             // Make sure all thumbnails are visible (no hiding individual thumbnails)
             for thumbnailView in videos[selectedIndex].thumbnailViews {
                 thumbnailView.isHidden = false
             }
             
             // Keep content view at original position - no shifting
             let contentView = selectedContainer.getContentView()
             contentView.frame = CGRect(
                 x: 0, // Keep content at original position
                 y: 0,
                 width: originalWidth, // Keep original content width for all thumbnails
                 height: selectedContainer.frame.height
             )
             
             // Container's clipsToBounds will automatically clip thumbnails extending beyond left boundary
             // Result: Thumbnail 1 gets progressively clipped, then fully clipped as ear moves right
             // Thumbnails 2,3,4,5 remain fully visible until ear reaches them
             
             // Update crop amount in data model (like right ear)
             let currentLeftEarEndX = clampedLeftX + 20 // Where content actually starts after left ear
             ...
             print("LeftEar Debug - Container frame: x=\(selectedContainer.frame.origin.x), width=\(selectedContainer.frame.width)")
             print("LeftEar Debug - Selection frame: x=\(selectionView.frame.origin.x), width=\(selectionView.frame.width)")
     ...
     private func repositionVideoContainersAfterTrimming(selectedIndex: Int) {
         var currentX: CGFloat = 25 // Start with 25px padding before first container
         
         for (index, container) in videoContainers.enumerated() {
             if index < selectedIndex {
                 // Keep positions for containers before selected (they should be properly positioned from initial layout)
                 currentX = container.frame.origin.x + container.frame.width
             } else if index == selectedIndex {
                 // Selected container already positioned by trimming logic, get its end
                 currentX = container.frame.origin.x + container.frame.width
             } else {
                 // Reposition containers after selected to follow immediately
